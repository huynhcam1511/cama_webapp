...
async function run() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (r[1] && r[1].includes('h')) {
      const parts = r[1].replace(' ', '').split('h');
      timeStr = `${parts[0].padStart(2, '0')}:${(parts[1] || '00').padStart(2, '0')}:00`;
    }
    ...
    const payload = {
      ...
      event_type: r[8] || 'CUSTOMER_APPOINTMENT',
      date: dateStr,
      start_time: timeStr,
      end_time: timeStr,
      customer_phone: r[2] !== '/' ? r[2] : null,
      customer_name: r[3],
      source: r[4],
      // PIC handled below if needed, we'll just skip UUID link for now
      service_group: r[6],
      service_content: r[7],
      status: r[9] || 'Chưa cập nhật',
      result: r[10] || 'Chưa cập nhật',
      next_follow_up: r[11] !== '/' ? r[11] : null,
      wedding_date: weddingDateStr,
      notes_before: r[13],
      notes_after: r[14] !== '/' ? r[14] : null
    };

    inserts.push(payload);
  }

  console.log("Parsed " + inserts.length + " rows to insert.");

  const response = await fetch(`${url}/rest/v1/operation_schedules`, {
    method: 'POST',
    headers: {
      'apikey': key,
  ...
  if (!response.ok) {
    ...
    console.log("Successfully imported 27 bookings!");
  }
}
