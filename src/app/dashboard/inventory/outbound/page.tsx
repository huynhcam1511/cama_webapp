...
import { Loader2, PackageMinus, MapPin, Search, CheckCircle2, QrCode, ArrowLeft, Shirt, AlertCircle } from "lucide-react";
import QRScanner from "@/components/qr-scanner";
import { useRouter } from "next/navigation";

export default function SimplifiedOutboundPage() {
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">Quét mã vị trí kệ {'->'} Chọn sản phẩm {'->'} Xuất</p>
        </div>
        
        {scannedLocation && (
          <button onClick={resetScanner} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl flex items-center gap-2 transition-colors">
            <ArrowLeft size={18} /> Đổi Vị Trí Kệ
          </button>
        )}
      </div>
