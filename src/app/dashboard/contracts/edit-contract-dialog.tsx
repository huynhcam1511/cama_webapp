...
export default function EditContractDialog({
  ...
  useEffect(() => {
    if (contract && isOpen) {
      setPaperContractNumber(contract.paper_contract_number || '');
      setContractDate(contract.contract_date || '');
      setBranch(contract.branch || '');
      setAssignedStaffName(contract.assigned_staff_name || '');
  ...
  const handleSubmit = async () => {
    ...
    let totalAmt = Number(payload.subtotal_amount) + Number(payload.surcharge_amount);
    if (payload.discount_type === "AMOUNT") totalAmt -= Number(payload.discount_amount);
    if (payload.discount_type === "PERCENT") totalAmt -= (Number(payload.subtotal_amount) * Number(payload.discount_amount)) / 100;
    payload.total_amount = totalAmt;

    try {
      const res = await updateContract(contract?.id, payload);
      setLoading(false);
      ...
      if (res.success) {
        ...
        setErrorMsg(res.error || "Không thể cập nhật hợp đồng, vui lòng thử lại.");
      }
    } catch (err: any) {
      setLoading(false);
