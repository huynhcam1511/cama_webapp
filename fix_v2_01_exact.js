...
function replaceLinesByIndex(startIdx, nextStartIdx, newCode) {
    let actualEnd = nextStartIdx;
    while(actualEnd > startIdx && lines[actualEnd - 1].trim() === '') {
        actualEnd--;
    }
    lines.splice(startIdx, actualEnd - startIdx, newCode);
}

// Ensure the array matches the indices
if (!lines[930].includes('export async function recordPaymentTransaction')) throw new Error('payment mismatch');
if (!lines[1108].includes('export async function cancelContract')) throw new Error('cancel mismatch');
if (!lines[1827].includes('export async function reserveContractInventory')) throw new Error('reserve mismatch');
...
replaceLinesByIndex(1827, 1942, reserveCode);
replaceLinesByIndex(1108, 1148, cancelCode);
replaceLinesByIndex(930, 1108, paymentCode);
...
console.log('actions.ts rewritten exactly by line indices');
