...
BEGIN
    ...
    IF NOT has_module_permission('INVENTORY_OUTBOUND', 'create') THEN
    ...
    LOOP
        ...
        -- Compute target status based on current status and reason
        IF v_reason = 'Giao khách' THEN
            IF v_current_status = 'RESERVED_SALE' THEN
                v_target_status := 'SOLD';
            ELSIF v_current_status = 'AVAILABLE' THEN
                v_target_status := 'RENTED';
            ELSE
                RAISE EXCEPTION 'Sản phẩm % đang ở trạng thái %, không thể xuất giao khách', v_instance_id, v_current_status;
            END IF;
        ELSE
    ...
    IF v_order_id IS NOT NULL AND v_reason = 'Giao khách' THEN
        UPDATE public.orders SET delivery_status = 'DELIVERED', updated_at = now() WHERE id = v_order_id;
    END IF;
