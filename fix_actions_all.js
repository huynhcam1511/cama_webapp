...
function replaceFunction(funcName, newBody, allowMissing = false) {
    ...
    if (endIdx === -1) {
        throw new Error(`End of function ${funcName} not found`);
    }

    content = content.substring(0, startIdx) + newBody + content.substring(endIdx);
}

// 1. replace reserveContractInventory
...
// 3. replace recordPaymentTransaction
const paymentCode = `export async function recordPaymentTransaction(
  ...
  const { error: updateError } = await supabase.from("contracts").update({ notes: stringifyMetadata(metaData), updated_at: new Date().toISOString() }).eq("id", contractId);
...
console.log('actions.ts rewritten fully');
