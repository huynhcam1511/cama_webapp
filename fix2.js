...
const endString = 'return { success: true };\n}';
let endIdx = content.indexOf(endString, startIdx);
if (endIdx === -1) {
  const endStringAlt = 'return { success: true };\r\n}';
  endIdx = content.indexOf(endStringAlt, startIdx);
  if (endIdx === -1) {
...
content = content.substring(0, startIdx) + replacement + content.substring(endIdx + endString.length + (content[endIdx + endString.length] === '\n' ? 0 : 0));
